...
@section('title', 'Cliente Final Detalle Día')

@section('content_header')
    <div class="d-flex justify-content-between">
        <h1>Cliente Final Detalle Día</h1>
        <a href="{{ route('admin.orders.final') }}" class="btn btn-outline-primary">Volver</a>
    </div>
@stop
            ...
            <table class="table table-hover table-dark">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Precio menu normal</th>
                ...
                <tbody>
                    <tr v-for="(item, index) in orders" :key="index">
                        ...
                        <td>@{{ formatPrice(item.associated.menu_normal_final) }}</td>
                        <td>@{{ item.count_menu_special }}</td>
                        <td>@{{ formatPrice(item.associated.menu_special_final) }}</td>
                        <td>@{{ formatPrice(item.total_price) }}</td>
                    </tr>
                </tbody>
            </table>
            <div class="d-flex justify-content-end">
                <a :href="`/pdf/final/detail-day/${day}`" target="_blank" class="btn btn-primary">Imprimir</a>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-footer d-flex justify-content-between">
                        ...
                        <a :href="`/pdf/final/detail/${order[0].order_number}/0`" target="_blank" class="btn btn-primary"
...
@section('js')
    <x-base-scripts />
    <script src="{{ asset('assets/js/admin/final-orders-day.js?v=' . time()) }}"></script>
@stop
