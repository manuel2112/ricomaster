...
@section('content_header')
    <div class="d-flex justify-content-between">
        <h1>Editar Minuta</h1>
        <a href="{{ route('admin.bill') }}" class="btn btn-outline-primary">Volver</a>
    </div>
@stop
                ...
                <tr v-for="item in listTypes" :key="item.id">
                    ...
                    <td :class="getBillByDay(item.id, week.monday) == '' ? 'bg-danger' : ''">
                        ...
                        <button type="button" class="btn btn-primary btn-block mt-2"
                            @click="openModal( item, getBillByMenu(item.id, week.monday), 0)">Editar</button>
                    </td>
                    <td :class="getBillByDay(item.id, week.tuesday) == '' ? 'bg-danger' : ''">
                        ...
                        <button type="button" class="btn btn-primary btn-block mt-2"
                            @click="openModal( item, getBillByMenu(item.id, week.tuesday), 1)">Editar</button>
                    </td>
                    <td :class="getBillByDay(item.id, week.wednesday) == '' ? 'bg-danger' : ''">
                        ...
                        <button type="button" class="btn btn-primary btn-block mt-2"
                            @click="openModal( item, getBillByMenu(item.id, week.wednesday), 2)">Editar</button>
                    </td>
                    <td :class="getBillByDay(item.id, week.thursday) == '' ? 'bg-danger' : ''">
                        ...
                        <button type="button" class="btn btn-primary btn-block mt-2"
                            @click="openModal( item, getBillByMenu(item.id, week.thursday), 3)">Editar</button>
                    </td>
                    <td :class="getBillByDay(item.id, week.friday) == '' ? 'bg-gradient-danger' : ''">
                        ...
                        <button type="button" class="btn btn-primary btn-block mt-2"
                            @click="openModal( item, getBillByMenu(item.id, week.friday), 4)">Editar</button>
                    </td>
                </tr>
            </tbody>
        </table>
                            ...
                            <div class="row">
                                <div class="col-md-10">
                                    <div class="form-group">
                                        <select class="form-control" v-model="menu.id">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="number" class="form-control" v-model.trim="menu.counter">
                                    </div>
                                </div>
                            </div>
...
@section('js')
    <x-base-scripts />
    <script src="{{ asset('assets/js/admin/bill-edit.js?v=' . time()) }}"></script>
@stop
