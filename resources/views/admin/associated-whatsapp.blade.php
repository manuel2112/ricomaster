...
@section('title', 'Cliente Asociado WhatsApp')

@section('content_header')
    <div class="d-flex justify-content-between">
        <h1>Cliente Asociado WhatsApp</h1>
    ...
    <div id="app">
        ...
        <div v-if="associateds.length > 0">
            <table class="table table-hover table-dark">
                ...
                <tbody>
                    <tr v-for="(associated, index) in associateds" :key="index">
                        <td>@{{ associated.name }}</td>
                        <td>+@{{ associated.whatsapp }}</td>
                        <td>@{{ formatPrice(associated.mount) }}</td>
                        <td v-if="optMenu == 'SI'">
                            <div class="btn-group-toggle" data-toggle="buttons">
                                <label class="btn btn-outline-primary">
                                    <input type="checkbox" :value="associated.id" v-model="groupedFinals">ENVIAR
                                </label>
                            </div>
                        </td>
                        <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <button type="button" class="btn btn-outline-success" :class="{ active: associated.send }"
                                    @click="stateWhatsapp(associated)">SI</button>
                                <button type="button" class="btn btn-outline-danger" :class="{ active: !associated.send }"
                                    @click="stateWhatsapp(associated)">NO</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
@section('js')
    <x-base-scripts />
    <script src="{{ asset('assets/js/admin/associated-whatsapp.js?v=' . time()) }}"></script>
@stop
